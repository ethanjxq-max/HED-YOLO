...
class Detect(nn.Module):
    ...
    #通过分组（如8400个锚点分成8组，每组1050个），每组选TopK，再在组间选TopK，可以减少计算量
    @staticmethod
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        ...
        #c2：Box分支的中间通道数，取最大值确保有足够的表达能力
        #c3：Cls分支的中间通道数
        # cv2是BOX检测框，也就是说当reg_max=1的时候(YOLO26),经过两个3x3的卷积后输出4 * reg_max个通道，直接预测x1,y1,x2,y2或cx,cy,w,h
        # cv3是cls分支，采用了深度可分离卷积，yolo26改变地方，参数量减少，跟了一个逐点卷积
        ...
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
         #训练时：使用cv2/cv3（一对多）进行训练，提供丰富的梯度
         #推理时：使用one2one_cv2/one2one_cv3（一对一）进行推理，每个目标只产生一个预测框
        if end2end:
    ...
    def end2end(self, value):
        ...
        #输入：x是长度为nl（通常是3）的列表，每个元素形状为[B, C, H, W]
        #输出：boxes形状为[B, 4*reg_max, N]，其中N = H1*W1 + H2*W2 + H3*W3（所有锚点总数）
        #scores形状为[B, nc, N]
    def forward_head(
    ...
    def forward(
        ...
        return y if self.export else (y, preds)
            ...
            #_get_decode_boxes：将预测的偏移量转换为实际坐标
            #scores.sigmoid()：将分类分数转换为概率
            #将box和scores拼接在一起，形状为[B, 4 + nc, N]
    ...
    def _get_decode_boxes(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # 解码bbox：dfl(x["boxes"])将DFL分布转换为坐标值，然后乘以步长
