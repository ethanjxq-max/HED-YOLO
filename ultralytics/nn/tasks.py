...
from .Convmodules import *  # 自定义卷积类模块（借鉴 yolov11-conv 工程模式）
from .SPPmodules import *   # 自定义SPP类模块
...
def parse_model(d, ch, verbose=True):
    ...
    # ✅ 变种卷积模块（借鉴 yolov11-conv 工程模式，在此登记自定义类）
    conv_modules = frozenset({C3k2_DB,C3k2_DB_DADC,C3k2_TSAC,C3k2_MRB,SPDConv,DySample,TSM,DCNorm,P2AT,CAA,FEM,AFM,PAM,WDM,WSDM,LDTE,SGDE,EAUp})
    # 后续所有 __init__(c1, c2, n=1, ...) 的复合模块（带 n 参数）
    conv_repeat_modules = frozenset({C3k2_DB, C3k2_DB_DADC, C3k2_TSAC, C3k2_MRB})
    # ✅ 输出通道 == 输入通道的模块（归一化 / 统计侧路）
    same_ch_modules = frozenset({TSM, DCNorm, DySample})

    # ✅ 变种SPP模块
    spp_modules = frozenset({SAEAIF_SPPF,AIF_SPPF})
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # ✅ ASFF 检测头输入自适应跨尺度融合：f = [P3层, P4层, P5层]（三输入），
            #    args = [c2, level]，c2 为输出通道（= 该尺度检测头原输入通道），level = 0/1/2
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [[ch[x] for x in f], c2, *args[1:]]
        elif m in conv_modules:
            if m in same_ch_modules:
                # ✅ TSM / DCNorm：输出通道 == 输入通道（归一化/侧路不改变通道），不做 width 缩放；
                #    yaml 参数直接作为模块的选项（如 TSM 的 out_ch / init_scale）
                c1, c2 = ch[f], ch[f]
                args = [c1, *args]
            elif m is SGDE:
                # ✅ SGDE 多输入分支：f=[浅层特征层, 深层语义层] → (c1, c2, c_sem, ...)
                c1, c2 = ch[f[0]], args[0]
                if c2 != nc:
                    c2 = make_divisible(min(c2, max_channels) * width, 8)
                args = [c1, c2, ch[f[1]], *args[1:]]
            else:
                c1, c2 = ch[f], args[0]  # 输入通道、输出通道
                if c2 != nc:
                    c2 = make_divisible(min(c2, max_channels) * width, 8)
                if m in conv_repeat_modules:
                    args = [c1, c2, n, *args[1:]]  # 适配 __init__(c1, c2, n, ...)
                    n = 1
                else:
                    args = [c1, c2, *args[1:]]  # 适配 __init__(c1, c2, ...)
                if m is C3k2_MRB:
                    # ⚠ 关键：官方 C3k2 会把 legacy 置 False，使 Detect 的 cls 分支走 YOLO26 默认的
                    #   深度可分离卷积；MRB 是 C3k2 的替换件，必须保持同一检测头结构，
                    #   否则实验就变成"换了瓶颈块 + 换了检测头"两个变量（会多出 ~1.3M 参数）。
                    legacy = False
                    if scale in {"m", "l", "x"}:
                        args[3:4] = [True]  # 同官方：M/L/X 尺度下 c3k=True
        elif m in spp_modules:
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]  # 适配 __init__(c1, c2, ...)
        ...
        elif m is DetectPlus:
            # ✅ DetectPlus：YOLO26 推理头增强变体（保持 NMS-free 端到端 + DFL-free）
            #    yaml 参数：[nc, adapt, wide, o2o_topk]
