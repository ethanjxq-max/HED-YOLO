    ...
    def update(self, model):
        ...
        if self.enabled:
            ...
            for k, v in self.ema.state_dict().items():
                # ✅ 修复：只对"dtype 与模型侧完全一致的浮点张量"做 EMA 更新。
                #    原因：RT-DETR 等模型在训练中会产生 float64 缓冲张量，而 torch._foreach_lerp_
                #    要求两侧 dtype 一致且为 float，否则报 "expected dtype float for `end` but got double"。
                #    被跳过的都是非可学习的缓冲量，对 EMA 无影响。
