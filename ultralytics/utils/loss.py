...
_NWD_G = float(os.environ.get("YOLO_NWD_GAMMA", "0"))  # AD-NWD 各向异性强度（0=标准NWD）


def wasserstein_loss(
    pred_bboxes: torch.Tensor, target_bboxes: torch.Tensor, C: float = 0.1, gamma: float = None
) -> torch.Tensor:
    """归一化 Wasserstein 距离相似度（输入 xyxy 归一化坐标，返回 [0,1]，越大越相似）。

    AD-NWD 扩展（自研，YOLO_NWD_GAMMA 控制，默认 0 = 标准 NWD）：
        标准 NWD 的中心项 dx^2+dy^2 是各向同性的，与 IoU 对细长目标的敏感性不匹配
        （细长目标垂直长轴的偏移对 IoU 破坏远大于沿长轴偏移）。AD-NWD 用目标自身
        长宽比 lambda 调制中心项：沿长轴宽容（kappa<1）、垂直长轴严格（kappa>1），
        lambda=1（方形）时严格退化为标准 NWD。gamma 控制各向异性强度。
    """
    if gamma is None:
        gamma = _NWD_G
    ...
    if gamma > 0:
        w_bar = (p_w + t_w) * 0.5
        h_bar = (p_h + t_h) * 0.5
        lam = torch.maximum(w_bar, h_bar) / (torch.minimum(w_bar, h_bar) + 1e-7)
        sgn = torch.where(w_bar >= h_bar, 1.0, -1.0)  # +1 水平长条(x为长轴), -1 垂直长条
        kx = lam ** (-gamma * sgn)  # 长轴方向 -> kappa<1 (宽容)
        ky = lam ** (gamma * sgn)  # 短轴方向 -> kappa>1 (严格)
        w2 = (
            kx * (p_cx - t_cx) ** 2
            + ky * (p_cy - t_cy) ** 2
            + ((p_w - t_w) * 0.5) ** 2
...
class E2ELoss:
    ...
    def __init__(self, model: torch.nn.Module, loss_fn=v8DetectionLoss):
        ...
        # 一对一（推理）分支：官方每 GT 只保留 1 个正样本（tal_topk2=1）。
        # 允许检测头用 `o2o_topk` 属性覆盖 —— 仅用于诊断"稀疏监督"假设（论文不采用该改动）。
        _o2o_k = int(getattr(getattr(model, "model", [None])[-1], "o2o_topk", 1))
        self.one2one = loss_fn(model, tal_topk=7, tal_topk2=_o2o_k)
        self.updates = 0
        self.total = 1.0
        # init gain
        self.o2m = 0.8
